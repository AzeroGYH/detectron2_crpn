...
from .point_fast_rcnn import FastRCNNOutputLayers as point_FastRCNNOutputLayers
...
class TextPointsROIHeads(ROIHeads):
    """
    ...
    def _init_box_head(cls, cfg, input_shape):
        ...
        if quad_flag:
            box_predictor = quad_FastRCNNOutputLayers(cfg, box_head.output_shape)
        else:
            box_predictor = point_FastRCNNOutputLayers(cfg, box_head.output_shape)
        return {
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            if has_gt:
                ...
                for (trg_name, trg_value) in targets_per_image.get_fields().items():
                    if (trg_name.startswith("gt_") or trg_name.startswith("beziers") or trg_name.startswith("points")) and not proposals_per_image.has(trg_name):
