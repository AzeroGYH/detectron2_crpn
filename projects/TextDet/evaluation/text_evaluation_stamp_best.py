...
from .general.voc_eval_text import evaluate_best
...
class StampEvaluatorBest(DatasetEvaluator):
    """
...
def instances_to_coco_json_without_recs(instances, img_id):
    ...
    if points is not None:
        ...
        for score, box in zip( scores, boxes):
            ...
            polys = np.array(temp_poly).reshape(-1,2).tolist()
            # print(polys)
            results["polys"].append(polys)
            results["scores"].append(score)
