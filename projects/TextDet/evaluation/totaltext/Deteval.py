...
from evaluation.totaltext.polygon_fast import iod
from evaluation.totaltext.polygon_fast import area_of_intersection
from evaluation.totaltext.polygon_fast import area
...
def total_eval_main(input_dir):
    """
    Input format: y0,x0, ..... yn,xn. Each detection is separated by the end of line token ('\n')'
    """

    # input_dir = '../../txt'  # detection directory goes here
    gt_dir = '/data7/gyh/AdelaiDet/datasets/totaltext/Groundtruth/Polygon/Test'  # gt directory goes here
    fid_path = 'Deteval.txt'  # output text file directory goes here
    ...
    fid = open(fid_path, 'a')
    temp = ('Precision: %s, Recall: %s, F_score: %s\n' % (str(precision), str(recall), str(f_score)))
    fid.write(temp)
    fid.close()
    print(temp)

    det_only_methodMetrics = r"DETECTION_ONLY_RESULTS: precision: {}, recall: {}, hmean: {}, ap: {}".format(str(precision), str(recall), str(f_score), str(0.0))
    resDict = {'calculated':True,'Message':'','det_only_method': det_only_methodMetrics}
    return resDict

if __name__ == "__main__":
    input_dir = '/data7/gyh/AdelaiDet/datasets/totaltext/final/'
    resDict = total_eval_main(input_dir)
    print(resDict)