...
from evaluation.totaltext.Deteval import total_eval_main
...
class TextEvaluatorTOTAL(DatasetEvaluator):
    """
    ...
    def to_eval_format(self, file_path, temp_dir="temp_det_results", cf_th=0.5):
        ...
        with open(file_path, 'r') as f:
            data = json.load(f)
            random_t = str(random.randint(1,100))
            with open(random_t+'temp_all_det_cors.txt', 'w') as f2:
                for ix in range(len(data)):
                    if data[ix]['score'] > 0.1:
                        ...
                        for i in range(len(data[ix]['polys'])):
                            outstr = outstr + str(int(data[ix]['polys'][i][1])) +','+str(int(data[ix]['polys'][i][0])) +','
                        outstr = outstr + str(round(data[ix]['score'], 3)) +',####'+'null'+'\n'	
                        f2.writelines(outstr)
                f2.close()
        dirn = temp_dir
        lsc = [cf_th] 
        fres = open(random_t+'temp_all_det_cors.txt', 'r').readlines()
        for isc in lsc:	
    ...
    def sort_detection(self, temp_dir):
        ...
        os.remove(self.temp_dataset_save_path + random_d +'_det.zip')
    ...
    def evaluate(self):
        ...
        text_result = total_eval_main(result_path)
        # os.remove(result_path)
...
def _convert_masks(masks_or_polygons):
    """
    ...
    Returns:
        list[GenericMask]:
    """
    height, width = masks_or_polygons.shape[1], masks_or_polygons.shape[2]
    m = masks_or_polygons
...
def instances_to_coco_json_without_recs(instances, img_id):
    if instances.has("pred_masks"):
        ...
        binary_mask = pred_masks.astype("uint8")  # opencv needs uint8
        mask = _convert_masks(binary_mask)
        polys = [m.polygons for m in mask]
        
        results = []
        if polys is not None:
            for poly, score, box in zip(polys, scores, boxes):
                if len(poly) == 0:
                    print("poly",len(poly))
                    continue
