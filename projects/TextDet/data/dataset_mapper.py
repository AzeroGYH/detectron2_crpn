...
class DatasetMapperWithBasis(DatasetMapper):
    ...
    def __call__(self, dataset_dict):
        ...
        # print("image_name",dataset_dict["file_name"])
        ...
        # rgb_image = utils.convert_image_to_rgb(image, self.image_format)
        # rgb_image = Image.fromarray(
        #     rgb_image.astype('uint8')).convert('RGB')

        # draw = ImageDraw.Draw(rgb_image)

        # beziers_h = dataset_dict['instances'].beziers
        # points_h = dataset_dict['instances'].points
        # gt_boxes_h = dataset_dict['instances'].gt_boxes.tensor

        # # beziers_h = beziers_h.reshape(-1, 2).tolist()
        # # new_beziers = []
        # # for beziers_ in beziers_h:
        # #     new_beziers.append(tuple(beziers_))
        # # draw.point(new_beziers, fill=(0, 255, 0))
        
        # points_h = points_h.tolist()
        # # print(points_h)
        
        # for points_ in points_h:
        #     new_points = []
        #     points_ = np.array(points_).reshape(-1,2)
        #     for p in points_:
        #         new_points.append(tuple(p))
        #     draw.polygon(new_points, outline=(255, 0, 0))


        # # gt_boxes_h = gt_boxes_h.tolist()
        # # for gt_boxes_ in gt_boxes_h:
        # #     new_d = [(gt_boxes_[0], gt_boxes_[1]), (gt_boxes_[2], gt_boxes_[
        # #         1]), (gt_boxes_[2], gt_boxes_[3]), (gt_boxes_[0], gt_boxes_[3])]
        # #     draw.polygon(new_d, outline=(255, 0, 0))

        # rgb_image.save(
        #     '/home/gyh/temp_imgs/{}'.format(dataset_dict["file_name"].split('/')[-1]))
